...
A list of free online programming books, categorized by languages/topics
  ...
  db.collection("contests").insertMany(myobj1, function(err, res) {
    if (err) throw err;
    console.log("document1 inserted");
    db.close();
  });
  db.collection("names").insertMany(myobj2, function(err, res) {
    if (err) throw err;
    console.log("document2 inserted");
    db.close();
  });
});